...
Fixpoint is_reachableb g q1 q2 n :=
  if State_eq_dec q1 q2 then true
    ...
    match n with
    | 0 => false
    | S n => existsb (fun q3 => is_reachableb g q3 q2 n) (flat_map (fun a => transitionf g [q1] a) (alphabet g))
    end.
...
Proof.
  ...
  - generalize dependent q1;
    induction n as [|n IH];
    intros q1 H; simpl in H;
    destruct (State_eq_dec q1 q2) as [H0|H0].
    1,3: subst; exists nil; split; constructor.
    apply le_0_n.
    discriminate.
    apply existsb_exists in H; destruct H as [q3 [H H1]].
    apply IH in H1; clear IH; destruct H1 as [w [H1 H2]].
    apply in_flat_map in H; destruct H as [a [H H3]].
    exists (a::w); split.
    simpl; nia.
    apply path_trans with q3.
    apply transition_transitionf in H3.
    1,2: intuition.
  - destruct H as [w [H H0]].
    generalize dependent n;
    induction H0;
    intros [|n] H1.
    1,2: simpl; destruct (State_eq_dec q q) as [H|H]; intuition.
    simpl in H1; nia.
    simpl; simpl in H1.
    destruct (State_eq_dec q1 q3) as [H2|H2].
    intuition.
    apply existsb_exists; exists q2; split.
    apply in_flat_map; exists a; split.
    apply transition_states_alphabet in H; intuition.
    apply transition_transitionf; intuition.
    apply IHpath; nia.
Qed.
